...
date_onset_db <- ncovdb_out_hubei %>% 
  filter(as.numeric(country)<3) %>% 
  count(country,fecha_ini_epiweek_w_fct) %>% 
  arrange(country,fecha_ini_epiweek_w_fct) %>% 
  rename(case_incidence=n) %>% 
  group_by(country) %>% 
  mutate(case_cumulative=cumsum(case_incidence)) %>% 
  ...
  pivot_longer(cols = c(case_incidence,case_cumulative),
               names_to = "case_type",
               values_to = "case_number") %>% 
  mutate(date_type="fecha_ini_epiweek_w_fct") %>% 
  rename(date="fecha_ini_epiweek_w_fct")

date_confirmed_db <- ncovdb_out_hubei %>% 
  filter(as.numeric(country)<3) %>% 
  count(country,fecha_conf_epiweek_w_fct) %>% 
  arrange(country,fecha_conf_epiweek_w_fct) %>% 
  rename(case_incidence=n) %>% 
  group_by(country) %>% 
  mutate(case_cumulative=cumsum(case_incidence)) %>% 
  ungroup() %>% 
  pivot_longer(cols = c(case_incidence,case_cumulative),
               names_to = "case_type",
               values_to = "case_number") %>% 
  mutate(date_type="fecha_conf_epiweek_w_fct") %>% 
  rename(date="fecha_conf_epiweek_w_fct")


# incidencia diaria -------------------------------------------------------

f02 <- union_all(date_onset_db,date_confirmed_db) %>% 
  mutate(date_type=fct_relevel(date_type,"fecha_ini_epiweek_w_fct")) %>% 
  filter(case_type=="case_incidence") %>%
  mutate(date_type=fct_recode(
    .f = date_type,
    "Inicio de síntomas"="fecha_ini_epiweek_w_fct",
    "Confirmación de caso"="fecha_conf_epiweek_w_fct")) %>% 
  ggplot(aes(x = date,y = case_number,fill=date_type)) +
  geom_col(position = position_dodge(preserve = "single")) +
  scale_fill_viridis_d(option = "cividis") +
  facet_wrap(~country,scales = "free",nrow = 1) +
  ...
  labs(fill="",x="Semana Epidemiológica",y="Casos incidentes")
...
f03 <- union_all(date_onset_db,date_confirmed_db) %>% 
  ...
  mutate(date_type=fct_recode(
    ...
    "Inicio de síntomas"="fecha_ini_epiweek_w_fct",
    "Confirmación de caso"="fecha_conf_epiweek_w_fct")) %>% 
  ggplot(aes(x = date,y = case_number,color=date_type,group=date_type)) +
  geom_line() +
  geom_point() + 
  scale_color_viridis_d(option = "cividis") +
  facet_wrap(~country,scales = "free",nrow = 1) +
  ...
  labs(color="",x="Semana Epidemiológica",y="Casos acumulados")
...
f02 / f03 / f01 + patchwork::plot_annotation(tag_levels = "A")
ggsave("figure/fig02-curve_week-incidence_cumulative_delay.png",height = 9,width = 7,dpi = "retina")
